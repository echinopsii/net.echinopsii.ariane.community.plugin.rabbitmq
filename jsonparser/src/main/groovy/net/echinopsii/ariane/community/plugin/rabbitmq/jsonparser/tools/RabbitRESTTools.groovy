...
class RabbitRESTTools {
    ...
    private transient static final Logger log = LoggerFactory.getLogger(RabbitRESTTools.class);

    static List<String> getConnectionNames(RabbitNodeToConnect node) {
        List<String> ret = new ArrayList<String>()
        try {
            def connections_list_req = node.get('/api/connections')
            ...
            if (log.isDebugEnabled())
                e.printStackTrace();
            log.error("PB with node " + node.getName() + " (" + node.getUrl() + "):" + e.getMessage())
        }
        return ret;
    }
    ...
    static List<String> getConnectionNames(RabbitClusterToConnect cluster) {
        ...
        try {
            ...
            if (log.isDebugEnabled())
                e.printStackTrace();
            log.error("PB with cluster " + cluster.getName() +
                    " (" + cluster.getSelectedNodeForREST().getUrl() + "):" + e.getMessage())
        }
        return ret;
    }
    ...
    static List<String> getChannelNames(RabbitNodeToConnect node) {
        List<String> ret = new ArrayList<String>()
        try {
            def channels_list_req = node.get('/api/channels')
    ...
    static Map<String,List<String>> getExchangeNames(RabbitNodeToConnect node) {
        ...
        try {
            def exchanges_list_req = node.get('/api/exchanges')
    ...
    static Map<String, List<String>> getBindingNames(RabbitNodeToConnect node) {
        Map<String,List<String>> ret = new HashMap<String, List<String>>()
        try {
            def bindings_list_req = node.get('/api/bindings')
    ...
    static Map<String,List<String>> getQueueNames(RabbitNodeToConnect node) {
        Map<String,List<String>> ret = new HashMap<String, List<String>>()
        try {
            def queues_list_req = node.get('/api/queues')
    ...
    static List<String> getVhostNames(RabbitNodeToConnect node) {
        List<String> ret = new ArrayList<String>()
        try {
            def vhosts_list_req = node.get('/api/vhosts')
